...
parallelAnalysis <- function(corrMatrix, observations = nrow(the_data), file_suffix){
  pdf(paste(paste(file.path(output_dir, "generatedPlots-CFA/ScreePlot-"),file_suffix,sep=""),'.pdf'), width=8, height=4)
...
fitMeasures_output <- function(modelFit, model_name) {
  ...
  write.csv(fit_Indices,file= file.path(output_dir, (paste("generatedData-CFA/fit_indices-", model_name, ".csv", sep=""))))
...
UX_factors <- list(
  visualization = list('visFaithful','visSimpleUnderstand','visAppropriate','sysRetrieveInfo', 'visRemember','trialError'),
  cognitive = list('sysThinkDeep', 'sysUseful', 'sysEnjoy', 'tasksCompleteTask'),
  intuitiveness = list('sysSimpleUnderstand', 'sysEasyStart', 'analysisConfigure')
)
...
generated_data_dir  <- file.path(output_dir, "generatedData-CFA")
generated_plots_dir  <- file.path(output_dir, "generatedPlots-CFA")
...
write.csv(corrM, file.path(output_dir, "generatedData-CFA/subscales-correlation.csv"))


## Save as PDF
this_file <- file.path(output_dir, "generatedPlots-CFA/subscales-corr.pdf")
...
write.csv(reliability_df_full, file.path(output_dir, "generatedData-CFA/reliability-subscales.csv"))
```
...
write.csv(reliab_df, file.path(output_dir, "generatedData-CFA/reliability-global-by-stimuli.csv"))
