...
make_corrM_PREVIS <- function(d, order = NULL, nb_fa = 4){
...
make_corrM_reusing_UXVIS <- function(path = "../Data_Analysis/polychoric_correlation_matrix_r.csv", order = NULL) {
  
 
  if (!file.exists(path)) {
    stop("Arquivo da matriz de correlação não encontrado em: ", path)
  }
  
  # 2. Ler o arquivo (suporta CSV ou RDS)
  if (grepl("\\.rds$", path, ignore.case = TRUE)) {
    r_mat <- readRDS(path)
  } else {
    # Lê o CSV garantindo nomes originais e definindo a primeira coluna como nomes de linha
    r_df <- read.csv(path, row.names = 1, check.names = FALSE)
    r_mat <- as.matrix(r_df)
  }
  
  # Garante que seja matriz numérica
  r_mat <- as.matrix(r_mat)
  
  # 3. Aplicar reordenação das variáveis se fornecida
  if (!is.null(order) && length(order) > 0) {
    # Filtra apenas itens válidos para evitar erro de índice fora de alcance
    valid_order <- intersect(order, rownames(r_mat))
    r_mat <- r_mat[valid_order, valid_order]
  }
...
make_corrM <- function(d = NULL, order = NULL, ...) {
  # 1. Tratamento caso d seja nulo
  if (is.null(d)) {
    if (exists("r_mat")) return(r_mat)
    return(NULL)
  }
  
  # 2. Remove itens com variância zero no subconjunto atual
  valid_cols <- sapply(d, function(col) length(unique(na.omit(col))) > 1)
  d_clean <- d[, valid_cols, drop = FALSE]
  
  if (ncol(d_clean) < 2) return(NULL)
  
  # 3. Converte para fatores ordenados
  d_ordered <- as.data.frame(lapply(d_clean, ordered))
  
  # 4. Calcula correlação policórica com pairwise deletion
  het <- tryCatch({
    polycor::hetcor(
      d_ordered, 
...
grp_data <- read.csv(paste("../Data/multigroup_ratings.csv"), encoding="UTF-8")
...
# Legacy function preserved for comparison
make_corrM_PREVIS <- function(d, order = NULL, nb_fa = 4) {
  if (anyNA(d)) {
    mi <- invisible(mifa(
      data      = d, 
    ...
    corr_MICE <- cov2cor(mi$cov_combined)
    if (length(order) > 0) {
      corr_MICE <- corr_MICE[order, order]
    }
    return(corr_MICE)
  } else {
    return(cor(d))
...
# Robust polychoric correlation builder with zero-variance handling
make_corrM <- function(d, order = NULL, ...) {
  # Drop items with zero variance in the current subset
  valid_cols <- sapply(d, function(col) length(unique(na.omit(col))) > 1)
  d_clean <- d[, valid_cols, drop = FALSE]
...
# Legacy reliability builder preserved for comparison
make_reliability_df_PREVIS <- function(this_data, col_name, row_suffix = "") {
  ...
  corrM_sub <- make_corrM_PREVIS(this_data, nb_fa = 1)
  om <- omega(corrM_sub, n.obs = nb_obs, 1)
...
write.csv(reliability_df_full, file = "generatedData-CFA/reliability-subscales.csv")
```
...
fitMeasures_output(fit_overall_pooled, paste0(this_model, "-POOLED_OVERALL"))
...
for (letter in stimuli_letters) {
  ...
  if (nrow(the_data) == 0) {
    message("Sem dados para o estímulo: ", letter)
    next
  }
  ...
  # 1. Executar checagens de normalidade e adequação protegidas
  tryCatch({
    stimulus_data_test(subset_data, letter)
  }, error = function(e) {
    message("Aviso nos testes preliminares para ", letter, ": ", e$message)
  })
  
  # 2. Confiabilidade por número de fatores
  this_alpha <- NA_real_
  this_om_1f <- NA_real_
  this_om_4f <- NA_real_
  ...
  for (nb_f in nbs_factors) {
    ...
    tryCatch({
      ...
      if (!is.null(c_mat) && ncol(c_mat) >= 2) {
        # Suavização para matrizes não positivas definidas em subamostras
      ...
      message("Erro ao calcular Omega (", nb_f, " fatores) em ", letter, ": ", e$message)
    })
  }
  
  # Armazenar resultados de confiabilidade da coluna
  reliab_df[[paste(letter, "- factors")]] <- c(this_alpha, this_om_1f, this_om_4f)
  
  # 3. CFA individual por estímulo
  ...
  tryCatch({
    ...
    message("Erro na CFA para o estímulo ", letter, ": ", e$message)
  })
}

# Salvar tabela de confiabilidade por estímulo
write.csv(reliab_df, file = "generatedData-CFA/reliability-global-by-stimuli.csv")
